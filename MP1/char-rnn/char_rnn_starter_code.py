...
def generate_abstract(model, device, file_name, temperature=0.8):
    # get list of titles from val set
    text = unidecode.unidecode(open(os.path.join(DATASET_PATH, "val.txt")).read())
    titles = [line for line in text.split("\n") if line[:6] == "Title:"]
    
    # get random indices for pulling titles from val set
...
def main():
    ...
    # Following function generates abstracts for titles in the val set for
    ...
    # test_prefix_strings = torch.load('test_prefix_strings.torch')
    test_prefix_strings = torch.load(os.path.join(DATASET_PATH, 'test_prefix_strings.torch'))
    prefix_inds = []
    for s in test_prefix_strings:
        prefix_inds.append(torch.tensor(list(map(lambda x: char_to_index_dict.get(x, UNK), s))))

    char_preds = []
    for inp in prefix_inds:
        with torch.no_grad():
            hidden = model.init_hidden(1, device)
            preds,_ = model(inp.unsqueeze(0).to(device),hidden)
            char_preds.append(preds[0,-1])

    char_preds = torch.stack(char_preds)
    char_preds = char_preds.detach().cpu().numpy()
    np.save('MP1_char_predictions',char_preds)
