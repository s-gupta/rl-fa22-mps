...
# Generate predictions on a data split
def predict(model,imset,device):
    val_dataset = SegmentationDataset(split=imset)
    val_dataloader = data.DataLoader(val_dataset, batch_size=1, 
                                     shuffle=False, num_workers=0, 
                                     drop_last=False)
...
def main():
    ...
    # performance of your model, you should upload to the autograder to be
    # evaluated against the test set.
    aps, ious = val(model, 'val', device, 'output.pdf')

    # Save out predictions for autograder, as uint8 to save space
    preds = predict(model,'test', device)
    np.save('MP1_sseg_predictions',(preds*255).astype(np.uint8))
